...
*******************************************************/
Crate.prototype.draw = function (c,s) {
  ...
  this.diagram.fillText("F",this.gForce.x*s+10+this.diagram.canvas.width/2,this.gForce.y*s+this.diagram.canvas.height/2);
  this.diagram.font = "10px Verdana";
  this.diagram.fillText("g",this.gForce.x*s+14+this.diagram.canvas.width/2,this.gForce.y*s+2+this.diagram.canvas.height/2);
  this.nForce.scale=s;
  this.nForce.draw(this.diagram);
  if(this.nForce.magnitude()!==0){
	  this.diagram.font = "14px Verdana";
	  this.diagram.fillStyle = this.nForce.color;
	  this.diagram.fillText("F",this.nForce.x*s+10+this.diagram.canvas.width/2,this.nForce.y*s+this.diagram.canvas.height/2);
	  this.diagram.font = "10px Verdana";
	  this.diagram.fillText("N",this.nForce.x*s+14+this.diagram.canvas.width/2,this.nForce.y*s+4+this.diagram.canvas.height/2);
  }
  //////console.log(this.nForce.magnitude());
  this.sFriction.scale = s;
  this.sFriction.draw(this.diagram);
  if(this.sFriction.magnitude()!==0){
	  this.diagram.font = "14px Verdana";
	  this.diagram.fillStyle = this.sFriction.color;
	  this.diagram.fillText("f", this.sFriction.x*s + 10 +this.diagram.canvas.width/2, this.sFriction.y*s-10+this.diagram.canvas.height/2);
	  this.diagram.font = "10px Verdana";
	  this.diagram.fillText("s", this.sFriction.x*s + 14 +this.diagram.canvas.width/2, this.sFriction.y*s-6+this.diagram.canvas.height/2);
  }
  this.kFriction.scale = s;
  this.kFriction.draw(this.diagram);
  if(this.kFriction.magnitude()!==0){
	  this.diagram.font = "14px Verdana";
	  this.diagram.fillStyle = this.kFriction.color;
	  this.diagram.fillText("f", this.kFriction.x*s + 10 +this.diagram.canvas.width/2, this.kFriction.y*s-10+this.diagram.canvas.height/2);
	  this.diagram.font = "10px Verdana";
	  this.diagram.fillText("k", this.kFriction.x*s + 14 +this.diagram.canvas.width/2, this.kFriction.y*s-6+this.diagram.canvas.height/2);
  }
  for(var p=0;p<this.appliedForces.length;p++){
	  //////console.log("harry");
	  this.appliedForces[p].draw(c);
	 // ////console.log(this.diagram);
	  this.aForces[p].scale =s;
	  this.aForces[p].draw(this.diagram);
	  this.diagram.font = "14px Verdana";
	  this.diagram.fillStyle = this.aForces[p].color;
	  this.diagram.fillText("F", this.aForces[p].x*s + this.aForces[p].offset.x*s +10 + this.diagram.canvas.width/2, this.aForces[p].y*s + this.aForces[p].offset.y*s+this.diagram.canvas.height/2);
	  this.diagram.font = "10px Verdana";
	  if(this.appliedForces[p].tension){
		  this.diagram.fillText("T",  this.aForces[p].x*s + this.aForces[p].offset.x*s +12 + this.diagram.canvas.width/2, 
									this.aForces[p].y*s + this.aForces[p].offset.y*s+4+this.diagram.canvas.height/2);
	  }
	  this.diagram.fillText(p+1, this.aForces[p].x*s + this.aForces[p].offset.x*s +18+ this.diagram.canvas.width/2, this.aForces[p].y*s + this.aForces[p].offset.y*s+4+this.diagram.canvas.height/2);
	 // ////console.log(this.aForces[p].offset);
	  
  }
  for(var k in this.cForces){
	  this.cForces[k].scale=s;
	  this.cForces[k].draw(this.diagram);
	  if(this.cForces[k].magnitude()!==0){
		  if(k==0){
			this.diagram.font = "14px Verdana";
			this.diagram.fillStyle = this.cForces[k].color;
			this.diagram.fillText("F", this.cForces[k].x*s+ this.cForces[k].offset.x*s+10+this.diagram.canvas.width/2, this.cForces[k].y*s + this.cForces[k].offset.y*s+this.diagram.canvas.height/2);
			this.diagram.font = "10px Verdana";
			this.diagram.fillText("T", this.cForces[k].x*s + this.cForces[k].offset.x*s+ 14 + this.diagram.canvas.width/2, this.cForces[k].y*s + this.cForces[k].offset.y*s+4+this.diagram.canvas.height/2);
		  }
		  else{
			this.diagram.font = "14px Verdana";
			this.diagram.fillStyle = this.cForces[k].color;
			this.diagram.fillText("F", this.cForces[k].x*s+ this.cForces[k].offset.x*s+10+this.diagram.canvas.width/2, this.cForces[k].y*s + this.cForces[k].offset.y*s+this.diagram.canvas.height/2);
			this.diagram.font = "10px Verdana";
			this.diagram.fillText("c", this.cForces[k].x*s+ this.cForces[k].offset.x*s + 14 + this.diagram.canvas.width/2, this.cForces[k].y*s + this.cForces[k].offset.y*s+4+this.diagram.canvas.height/2);
		  }
	  }
  }
